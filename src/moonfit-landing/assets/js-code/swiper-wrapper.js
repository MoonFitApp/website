(
	function( $ ) {
		...
		$.fn.MoonfitSwiper = function( options ) {
			var defaults = {};
			...
			this.each( function() {
				...
				$swiper.on( 'init', function() {
					...
					visibleSlide.addClass( 'moonfit-slide-active' );
				} );
				...
				$swiper.on( 'slideChangeTransitionStart', function() {
					var slides = $swiper.$wrapperEl.find( '.swiper-slide' );
					slides.removeClass( 'moonfit-slide-active' );
				} );
				...
				$( document ).trigger( 'MoonfitSwiperInit', [ $swiper, $slider, swiperOptions ] );
			} );
