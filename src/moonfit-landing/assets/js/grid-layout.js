(
	function( $ ) {
		...
		var MoonfitGridPlugin = function( $el, options ) {
			this.$el = $el;
			var $grid = $el.find( '.moonfit-grid' );
			...
			this.init = function() {
				...
				$el.on( 'MoonfitQueryEnd', function( event, el, $items ) {
					plugin.update( $items );
				} );
			};
				...
				if ( $.fn.laziestloader ) {
					...
					if ( llImages.length > 0 ) {
						llImages.laziestloader( {}, function() {
							$( this ).unwrap( '.moonfit-lazy-image' );
			...
			this.calculateMasonrySize = function( options = {} ) {
				...
				if ( oldActiveColumns !== plugin.activeColumns ) {
					$( document.body ).trigger( 'MoonfitGridLayoutColumnsChange', [
						$el,
						oldActiveColumns,
				...
				setTimeout( function() {
					...
					$el.trigger( 'MoonfitGridLayoutResized' );
				}, 250 );
			};
		...
		const namespace = 'moonfitGridLayout';

		$.fn.extend( {
			MoonfitGridLayout: function( args, update ) {
				...
				return this.each( function() {
					...
					if ( instance ) { // Already created then trigger method.
						...
						instance = new MoonfitGridPlugin( $el, args );
